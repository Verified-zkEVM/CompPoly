...
What the benchmark suite measures, how, and the best time recorded so far for
every operation it covers. Part 1 is the infrastructure. Part 2 is the record:
one section per benchmarked component, each ending in a table of current best
times, every row tied to the commit whose build produced it. An optimisation
pass that is kept overwrites the rows it improved; nothing else in this file
should need to change between passes.

Companion pages: [`bench/README.md`](bench/README.md) is the operator's guide
(invocation, presets, output files), [`docs/wiki/benchmarking.md`](docs/wiki/benchmarking.md)
holds the recurring guidance (sinks, reading a result, adding a group),
[`docs/wiki/autoresearch.md`](docs/wiki/autoresearch.md) is the optimisation
loop, and [`docs/bench-audit-2026.md`](docs/bench-audit-2026.md) is the frozen
audit and change log that produced all of this.

## Contents

- [Part 1. The infrastructure](#part-1-the-infrastructure)
  - [1.1 The suite](#11-the-suite)
  - [1.2 How a row is measured](#12-how-a-row-is-measured)
  - [1.3 What keeps a number honest](#13-what-keeps-a-number-honest)
  - [1.4 Where it runs](#14-where-it-runs)
  - [1.5 Comparing two builds](#15-comparing-two-builds)
  - [1.6 Where things live](#16-where-things-live)
- [Part 2. Components and current best times](#part-2-components-and-current-best-times)
  - [2.0 How to read and how to update the tables](#20-how-to-read-and-how-to-update-the-tables)
  - [2.1 Harness self-check](#21-harness-self-check)
  - [2.2 Base-field arithmetic](#22-base-field-arithmetic)
  - [2.3 Pairing scalar fields](#23-pairing-scalar-fields)
  - [2.4 Extension fields](#24-extension-fields)
  - [2.5 Binary tower fields](#25-binary-tower-fields)
  - [2.6 Additive NTT](#26-additive-ntt)
  - [2.7 Univariate evaluation and remainder](#27-univariate-evaluation-and-remainder)
  - [2.8 Univariate multiplication](#28-univariate-multiplication)
  - [2.9 Multiplicative NTT and Reed-Solomon encoding](#29-multiplicative-ntt-and-reed-solomon-encoding)
  - [2.10 Univariate root finding](#210-univariate-root-finding)
  - [2.11 Multivariate and multilinear evaluation](#211-multivariate-and-multilinear-evaluation)
  - [2.12 Bivariate operations](#212-bivariate-operations)
  - [2.13 Guruswami-Sudan list decoding](#213-guruswami-sudan-list-decoding)
...
Every row is executed in two passes. The **validation pass** is untimed: it
folds a strong digest over the full result of every iteration in the body's
period, and every row of a group must produce the same digest, or the run
fails. This is a differential test between implementations, run on every PR,
and it is why a wrong-but-fast implementation cannot appear in a table here.
The **timed pass** folds each result through a cheap `sink` whose only job is to
keep the result live, and it produces the numbers in Part 2.

### 1.2 How a row is measured
...
A calibration ramp times 1, 2, 4, … iterations until the warmup budget is met;
the ramp is the warmup, and its last step estimates the per-iteration cost.
That estimate fixes how many iterations make one sample, and the cap bounds the
row's total. The reported number is the **median** sample in picoseconds per
iteration, with the median absolute deviation as the `Spread`. Outliers are
labelled by Tukey fences and never dropped. A row whose single iteration
exhausts the cap is `n=1` and carries no dispersion at all.

Operations of a nanosecond or two cannot be timed one per iteration, because
the loop itself costs about as much. The field and kernel groups therefore
**chain** the operation `workUnits` times per iteration and the report divides,
giving a **per-unit** cost. Two chain shapes are reported, named as Plonky3
names them: *latency*, where each operation depends on the last, and
*throughput*, with ten independent accumulators the pipeline can overlap.

### 1.3 What keeps a number honest

- **The digest.** Every row of a group agrees with every other on the full
  result, and a group's inputs derive from its own key, so digests are also
  comparable across runs, presets, and commits. A digest that changes between
  two builds means the code computes something else.
- **The harness self-check.** `harness-floor` times an empty body and
  `harness-canary` a body of known cost; the canary fails the run if it does
  not clear the floor. `harness-chain-floor` and `harness-chain-linearity` do
  the same for chained bodies. A body the compiler has folded away looks
  exactly like a body that got very fast, and these four groups are how the
  two are told apart.
- **The manifest.** Every run writes `manifest-<run>.json` with the commit, a
  dirty flag, the toolchain, the resolved budget, the seed, the selection and
  the host. A number without its manifest is not attributable, and none of the
  numbers below lacks one.
- **Declared iteration periods.** The digest length is the period of the body
  in its iteration index, a property of the benchmark, never of the machine.

### 1.4 Where it runs

Correctness gates every PR: `lean_action_ci.yml` runs
`CompPolyBench --medium --validate-only` over the curated set in
`bench/ci-groups.txt`, which does the digest pass and the agreement check and
collects no samples. It is deterministic and machine-independent, which is what
a gate must be.

Timings run on demand: `benchmarks.yml` via **Actions → Benchmarks → Run
workflow**, a `/bench` comment on a PR from a repo member, or automatically on a
PR touching `bench/**`. They are advisory because cross-run comparability on a
runner whose CPU changes between runs has not been validated. The numbers that
matter for optimisation are taken on one machine, by the method in 1.5.

### 1.5 Comparing two builds
...
The optimisation loop built on this is the subject of
[`docs/wiki/autoresearch.md`](docs/wiki/autoresearch.md): one change per
iteration, `lake build` as the proof gate, `bench-ab.sh run` as the measurement,
keep on `faster` without `SUSPECT`, revert otherwise. The trusted code base
does not move during it: a fast implementation is swapped in by `@[csimp]` with
an equality theorem, or by a twin definition with an `_eq_` theorem, never by
...
## Part 2. Components and current best times

### 2.0 How to read and how to update the tables
...
group at `--medium`; the `n=1` rows are workloads whose single iteration
exhausts the budget and are listed for completeness, not for comparison.

### 2.1 Harness self-check
...
### 2.7 Univariate evaluation and remainder
...
at this preset. *Peer:* none at this granularity; the field rows in 2.2 bound
...
### 2.9 Multiplicative NTT and Reed-Solomon encoding
...
the list decoder in 2.13 calls.
...
### 2.11 Multivariate and multilinear evaluation
...
### 2.13 Guruswami-Sudan list decoding
