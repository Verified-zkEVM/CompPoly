...
theorem fPoly_irreducible : Irreducible fPoly := by
  ...
  refine irreducible_of_rabin_prime_degree_of_card hcard (by norm_num) fPoly_natDegree ?_ ?_
...
theorem f6_irreducible : Irreducible f6 := by
  have hcard : Fintype.card (ZMod P) = P := ZMod.card P
  refine irreducible_of_rabin_degree_six_of_card hcard f6_natDegree ?_ ?_ ?_
...
`irreducible_of_rabin_prime_degree_of_card`, `irreducible_of_rabin_degree_six_of_card` and
`irreducible_of_rabin_two_prime_factors_of_card` and `irreducible_of_rabin_prime_power_of_card`
state their conditions at a caller-supplied numeral `q` with `hcard : Fintype.card F = q`, which
is the shape concrete extensions use. Their docstrings claim nothing is weakened; the four
theorems below are that claim, machine-checked.
Instantiating at `q := Fintype.card F` with `rfl` has to recover
...
/-- `irreducible_of_rabin_two_prime_factors_of_card` recovers
...
theorem two_prime_factors_recovered {F : Type*} [Field F] [Fintype F] {f : F[X]}
    ...
    (h_cop₂ : IsCoprime f (X ^ (Fintype.card F ^ (d / ℓ₂)) - X)) :
    Irreducible f :=
  irreducible_of_rabin_two_prime_factors_of_card rfl h_deg h_pos h_factors h_trace h_cop₁ h_cop₂

/-- `irreducible_of_rabin_prime_power_of_card` recovers `irreducible_of_rabin_prime_power`. -/
theorem prime_power_recovered {F : Type*} [Field F] [Fintype F] {f : F[X]} {d ℓ k : ℕ}
    (hℓ : ℓ.Prime) (hk : k ≠ 0) (hd_eq : d = ℓ ^ k) (h_deg : f.natDegree = d)
    (h_trace : f ∣ X ^ (Fintype.card F ^ d) - X)
    (h_cop : IsCoprime f (X ^ (Fintype.card F ^ (d / ℓ)) - X)) :
    Irreducible f :=
  irreducible_of_rabin_prime_power_of_card rfl hℓ hk hd_eq h_deg h_trace h_cop
