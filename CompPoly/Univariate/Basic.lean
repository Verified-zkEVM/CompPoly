...
-/
def CPolynomialC (R : Type*) [BEq R] [Ring R] := { p : CPolynomialRaw R // p.trim = p }
...
instance : Coe (CPolynomialC R) (CPolynomialRaw R) where coe := Subtype.val
...
theorem add_comm : p + q = q + p := by
  apply CPolynomialC.ext; apply CPolynomialRaw.add_comm

theorem add_assoc : p + q + r = p + (q + r) := by
  apply CPolynomialC.ext; apply CPolynomialRaw.add_assoc
...
theorem nsmul_zero : nsmul 0 p = 0 := by
  apply CPolynomialC.ext; apply CPolynomialRaw.nsmul_zero

theorem nsmul_succ (n : ℕ) (p : CPolynomialC R) : nsmul (n + 1) p = nsmul n p + p := by
  apply CPolynomialC.ext; apply CPolynomialRaw.nsmul_succ
