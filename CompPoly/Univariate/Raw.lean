...
/-!
  ...
  This file contains a computable datatype for univariate polynomial, `CPolynomialRaw R`. This is
...
def CPolynomialRaw (R : Type*) := Array R

namespace CPolynomialRaw

/-- Construct a `CPolynomialRaw` from an array of coefficients. -/
@[reducible]
def mk {R : Type*} (coeffs : Array R) : CPolynomialRaw R := coeffs

/-- Extract the underlying array of coefficients. -/
@[reducible]
def coeffs {R : Type*} (p : CPolynomialRaw R) : Array R := p
...
@[reducible]
def coeff (p : CPolynomialRaw Q) (i : ℕ) : Q := p.getD i 0

/-- The constant polynomial `C r`. -/
def C (r : R) : CPolynomialRaw R := #[r]

/-- The variable `X`. -/
def X : CPolynomialRaw R := #[0, 1]

/-- Construct a monomial `c * X^n` as a `CPolynomialRaw R`.
...
-/
def monomial [DecidableEq R] (n : ℕ) (c : R) : CPolynomialRaw R :=
...
/-- Return the index of the last non-zero coefficient of a `CPolynomialRaw` -/
def lastNonzero (p : CPolynomialRaw R) : Option (Fin p.size) :=
  p.findIdxRev? (· != 0)

/-- Remove leading zeroes from a `CPolynomialRaw`.
Requires `BEq` to check if the coefficients are zero. -/
def trim (p : CPolynomialRaw R) : CPolynomialRaw R :=
  match p.lastNonzero with
  | none => #[]
  | some i => p.extract 0 (i.val + 1)

/-- Return the degree of a `CPolynomialRaw`. -/
def degree (p : CPolynomialRaw R) : Nat :=
  match p.lastNonzero with
  | none => 0
  | some i => i.val + 1

/-- Natural number degree of a `CPolynomialRaw`.
...
theorem lastNonzero_none [LawfulBEq R] {p : CPolynomialRaw R} :
...
theorem lastNonzero_some [LawfulBEq R] {p : CPolynomialRaw R} {i} (hi : i < p.size) (h : p[i] ≠ 0) :
...
theorem lastNonzero_spec [LawfulBEq R] {p : CPolynomialRaw R} {k} :
...
def lastNonzeroProp {p : CPolynomialRaw R} (k : Fin p.size) : Prop :=
  p[k] ≠ 0 ∧ (∀ j, (hj : j < p.size) → j > k → p[j] = 0)

/-- The last non-zero index is unique. -/
lemma lastNonzero_unique {p : CPolynomialRaw Q} {k k' : Fin p.size} :
...
theorem lastNonzero_some_iff [LawfulBEq R] {p : CPolynomialRaw R} {k} :
...
/-- eliminator for `p.lastNonzero`, e.g. use with the induction tactic as follows:
  ...
  ```
-/
theorem lastNonzero_induct [LawfulBEq R] {motive : CPolynomialRaw R → Prop}
    (case1 : ∀ p, p.lastNonzero = none → (∀ i, (hi : i < p.size) → p[i] = 0) → motive p)
  (case2 : ∀ p : CPolynomialRaw R, ∀ k : Fin p.size, p.lastNonzero = some k → p[k] ≠ 0 →
    (∀ j : ℕ, (hj : j < p.size) → j > k → p[j] = 0) → motive p)
  (p : CPolynomialRaw R) : motive p := by
...
/-- eliminator for `p.trim`; use with the induction tactic as follows:
  ...
  ```
-/
theorem induct [LawfulBEq R] {motive : CPolynomialRaw R → Prop}
    (case1 : ∀ p, p.trim = #[] → (∀ i, (hi : i < p.size) → p[i] = 0) → motive p)
  (case2 : ∀ p : CPolynomialRaw R, ∀ k : Fin p.size, p.trim = p.extract 0 (k + 1)
    → p[k] ≠ 0 → (∀ j : ℕ, (hj : j < p.size) → j > k → p[j] = 0) → motive p)
  (p : CPolynomialRaw R) : motive p := by induction p using lastNonzero_induct with
  | case1 p h_none h_all_zero =>
...
/-- eliminator for `p.trim`; e.g. use for case distinction as follows:
  ...
  ```
-/
theorem elim [LawfulBEq R] (p : CPolynomialRaw R) :
...
theorem size_eq_degree (p : CPolynomialRaw R) : p.trim.size = p.degree := by
  unfold trim degree
  match h : p.lastNonzero with
  | none => simp
  | some i => simp [Fin.is_lt, Nat.succ_le_of_lt]

theorem size_le_size (p : CPolynomialRaw R) : p.trim.size ≤ p.size := by
  unfold trim
  match h : p.lastNonzero with
  | none => simp
...
theorem coeff_eq_getElem_of_lt [LawfulBEq R] {p : CPolynomialRaw R} {i} (hi : i < p.size) :
...
theorem coeff_eq_coeff [LawfulBEq R] (p : CPolynomialRaw R) (i : ℕ) :
...
theorem eq_of_equiv [LawfulBEq R] {p q : CPolynomialRaw R} : equiv p q → p.trim = q.trim := by
  unfold equiv
  intro h
  ext
...
theorem trim_equiv [LawfulBEq R] (p : CPolynomialRaw R) : equiv p.trim p := by
  apply coeff_eq_coeff

theorem trim_twice [LawfulBEq R] (p : CPolynomialRaw R) : p.trim.trim = p.trim := by
  apply eq_of_equiv
  apply trim_equiv

theorem canonical_empty : (CPolynomialRaw.mk (R:=R) #[]).trim = #[] := by
  have : (CPolynomialRaw.mk (R:=R) #[]).lastNonzero = none := by
    simp [lastNonzero]
    apply Array.findIdxRev?_empty_none
    rfl
  rw [trim, this]

theorem canonical_of_size_zero {p : CPolynomialRaw R} : p.size = 0 → p.trim = p := by
  intro h
  suffices h_empty : p = #[] by rw [h_empty]; exact canonical_empty
  exact Array.eq_empty_of_size_eq_zero h

theorem canonical_nonempty_iff [LawfulBEq R] {p : CPolynomialRaw R} (hp : p.size > 0) :
...
theorem lastNonzero_last_iff [LawfulBEq R] {p : CPolynomialRaw R} (hp : p.size > 0) :
...
theorem canonical_iff [LawfulBEq R] {p : CPolynomialRaw R} :
...
theorem non_zero_map [LawfulBEq R] (f : R → R) (hf : ∀ r, f r = 0 → r = 0) (p : CPolynomialRaw R) :
    let fp := CPolynomialRaw.mk (p.map f);
...
-/
theorem canonical_ext [LawfulBEq R] {p q : CPolynomialRaw R} (hp : p.trim = p) (hq : q.trim = q) :
    equiv p q → p = q := by
  intro h_equiv
  rw [← hp, ← hq]
...
/-- Evaluates a `CPolynomialRaw` at `x : S` using a ring homomorphism `f : R →+* S`.

  Computes `f(a₀) + f(a₁) * x + f(a₂) * x² + ...` where `aᵢ` are the coefficients.

  TODO: define an efficient version of this with caching -/
def eval₂ [Semiring S] (f : R →+* S) (x : S) (p : CPolynomialRaw R) : S :=
  p.zipIdx.foldl (fun acc ⟨a, i⟩ => acc + f a * x ^ i) 0

/-- Evaluates a `CPolynomialRaw` at a given value -/
@[inline, specialize]
def eval (x : R) (p : CPolynomialRaw R) : R :=
...
@[inline, specialize]
def addRaw (p q : CPolynomialRaw R) : CPolynomialRaw R :=
  let ⟨p', q'⟩ := Array.matchSize p q 0
  .mk (Array.zipWith (· + ·) p' q' )

/-- Addition of two `CPolynomialRaw`s, with result trimmed. -/
@[inline, specialize]
def add (p q : CPolynomialRaw R) : CPolynomialRaw R :=
  addRaw p q |> trim

/-- Scalar multiplication: multiplies each coefficient by `r`. -/
@[inline, specialize]
def smul (r : R) (p : CPolynomialRaw R) : CPolynomialRaw R :=
  .mk (Array.map (fun a => r * a) p)

/-- Raw scalar multiplication by a natural number (may have trailing zeros). -/
@[inline, specialize]
def nsmulRaw (n : ℕ) (p : CPolynomialRaw R) : CPolynomialRaw R :=
  .mk (Array.map (fun a => n * a) p)

/-- Scalar multiplication of `CPolynomialRaw` by a natural number, with result trimmed. -/
@[inline, specialize]
def nsmul (n : ℕ) (p : CPolynomialRaw R) : CPolynomialRaw R :=
  nsmulRaw n p |> trim

/-- Negation of a `CPolynomialRaw`. -/
@[inline, specialize]
def neg (p : CPolynomialRaw R) : CPolynomialRaw R := p.map (fun a => -a)

/-- Subtraction of two `CPolynomialRaw`s. -/
@[inline, specialize]
def sub (p q : CPolynomialRaw R) : CPolynomialRaw R := p.add q.neg

/-- Multiplication by `X^i`: shifts coefficients right by `i` positions (prepends `i` zeros). -/
@[inline, specialize]
def mulPowX (i : Nat) (p : CPolynomialRaw R) : CPolynomialRaw R := .mk (Array.replicate i 0 ++ p)

/-- Multiplication of a `CPolynomialRaw` by `X`, reduces to `mulPowX 1`. -/
@[inline, specialize]
def mulX (p : CPolynomialRaw R) : CPolynomialRaw R := p.mulPowX 1

/-- Multiplication using the naive `O(n²)` algorithm: `Σᵢ (aᵢ * q) * X^i`. -/
@[inline, specialize]
def mul (p q : CPolynomialRaw R) : CPolynomialRaw R :=
  p.zipIdx.foldl (fun acc ⟨a, i⟩ => acc.add <| (smul a q).mulPowX i) (mk #[])

/-- Exponentiation of a `CPolynomialRaw` by a natural number `n` via repeated multiplication. -/
@[inline, specialize]
def pow (p : CPolynomialRaw R) (n : Nat) : CPolynomialRaw R := (mul p)^[n] (C 1)

-- TODO: define repeated squaring version of `pow`

instance : Zero (CPolynomialRaw R) := ⟨#[]⟩
instance : One (CPolynomialRaw R) := ⟨CPolynomialRaw.C 1⟩
instance : Add (CPolynomialRaw R) := ⟨CPolynomialRaw.add⟩
instance : SMul R (CPolynomialRaw R) := ⟨CPolynomialRaw.smul⟩
instance : SMul ℕ (CPolynomialRaw R) := ⟨nsmul⟩
instance : Neg (CPolynomialRaw R) := ⟨CPolynomialRaw.neg⟩
instance : Sub (CPolynomialRaw R) := ⟨CPolynomialRaw.sub⟩
instance : Mul (CPolynomialRaw R) := ⟨CPolynomialRaw.mul⟩
instance : Pow (CPolynomialRaw R) Nat := ⟨CPolynomialRaw.pow⟩
instance : NatCast (CPolynomialRaw R) := ⟨fun n => CPolynomialRaw.C (n : R)⟩
instance : IntCast (CPolynomialRaw R) := ⟨fun n => CPolynomialRaw.C (n : R)⟩

/-- Upper bound on degree: `size - 1` if non-empty, `⊥` if empty. -/
def degreeBound (p : CPolynomialRaw R) : WithBot Nat :=
  match p.size with
  | 0 => ⊥
  | .succ n => n

/-- Convert `degreeBound` to a natural number by sending `⊥` to `0`. -/
def natDegreeBound (p : CPolynomialRaw R) : Nat :=
  (degreeBound p).getD 0

/-- Check if a `CPolynomialRaw` is monic, i.e. its leading coefficient is 1. -/
def monic (p : CPolynomialRaw R) : Bool := p.leadingCoeff == 1

/-- Division with remainder by a monic polynomial using polynomial long division. -/
def divModByMonicAux [Field R] (p : CPolynomialRaw R) (q : CPolynomialRaw R) :
    CPolynomialRaw R × CPolynomialRaw R :=
  go (p.size - q.size) p q
where
  go : Nat → CPolynomialRaw R → CPolynomialRaw R → CPolynomialRaw R × CPolynomialRaw R
  | 0, p, _ => ⟨0, p⟩
  | n+1, p, q =>
...
/-- Division of `p : CPolynomialRaw R` by a monic `q : CPolynomialRaw R`. -/
def divByMonic [Field R] (p : CPolynomialRaw R) (q : CPolynomialRaw R) :
    CPolynomialRaw R :=
  (divModByMonicAux p q).1

/-- Modulus of `p : CPolynomialRaw R` by a monic `q : CPolynomialRaw R`. -/
def modByMonic [Field R] (p : CPolynomialRaw R) (q : CPolynomialRaw R) :
    CPolynomialRaw R :=
  (divModByMonicAux p q).2

/-- Division of two `CPolynomialRaw`s. -/
def div [Field R] (p q : CPolynomialRaw R) : CPolynomialRaw R :=
  (C (q.leadingCoeff)⁻¹ • p).divByMonic (C (q.leadingCoeff)⁻¹ * q)

/-- Modulus of two `CPolynomialRaw`s. -/
def mod [Field R] (p q : CPolynomialRaw R) : CPolynomialRaw R :=
  (C (q.leadingCoeff)⁻¹ • p).modByMonic (C (q.leadingCoeff)⁻¹ * q)

instance [Field R] : Div (CPolynomialRaw R) := ⟨div⟩
instance [Field R] : Mod (CPolynomialRaw R) := ⟨mod⟩

/-- Pseudo-division by `X`: removes the constant term and shifts remaining coefficients left. -/
def divX (p : CPolynomialRaw R) : CPolynomialRaw R := p.extract 1 p.size

variable (p q r : CPolynomialRaw R)

-- some helper lemmas to characterize p + q

lemma matchSize_size_eq {p q : CPolynomialRaw Q} :
    let (p', q') := Array.matchSize p q 0
    p'.size = q'.size := by
...
lemma matchSize_size {p q : CPolynomialRaw Q} :
...
theorem add_size {p q : CPolynomialRaw Q} : (addRaw p q).size = max p.size q.size := by
...
theorem add_coeff {p q : CPolynomialRaw Q} {i : ℕ} (hi : i < (addRaw p q).size) :
    (addRaw p q)[i] = p.coeff i + q.coeff i := by
  simp [addRaw]
  by_cases hi' : i < p.size <;> by_cases hi'' : i < q.size <;> simp_all

theorem add_coeff? (p q : CPolynomialRaw Q) (i : ℕ) :
...
lemma add_coeff_trimmed [LawfulBEq R] (p q : CPolynomialRaw R) (i : ℕ) :
    (p + q).coeff i = p.coeff i + q.coeff i := by
      have h_add : p + q = (p.addRaw q).trim := by rfl
      have h_trim_coeff : ∀ (p : CPolynomialRaw R) (i : ℕ), p.coeff i = p.trim.coeff i := by
...
lemma add_equiv_raw [LawfulBEq R] (p q : CPolynomialRaw R) :
...
lemma mul_pow_assoc : ∀ (p : CPolynomialRaw R) (n : ℕ),
    ∀ (q : CPolynomialRaw R) (m l : ℕ),
  l + m = n →
...
lemma mul_pow_succ (p q : CPolynomialRaw R) (n : ℕ):
    p.mul^[n + 1] q = p.mul (p.mul^[n] q) := by
  rw [mul_pow_assoc p (n+1) q 1 n] <;> simp

omit [BEq R] in
lemma neg_coeff : ∀ (p : CPolynomialRaw R) (i : ℕ), p.neg.coeff i = - p.coeff i := by
  intro p i
  unfold neg coeff
  rcases (Nat.lt_or_ge i p.size) with hi | hi <;> simp [hi]

lemma trim_add_trim [LawfulBEq R] (p q : CPolynomialRaw R) : p.trim + q = p + q := by
  apply Trim.eq_of_equiv
  intro i
  rw [add_coeff?, add_coeff?, Trim.coeff_eq_coeff]

/-
CPolynomialRaw does in general form a Ring or any other 'nice' structure,
...
omit [Ring Q] in
@[simp] theorem zero_def : (0 : CPolynomialRaw Q) = #[] := rfl
...
def canonical (p : CPolynomialRaw R) := p.trim = p

theorem zero_canonical : (0 : CPolynomialRaw R).trim = 0 := Trim.canonical_empty
...
theorem zero_add_trim [LawfulBEq R] (p : CPolynomialRaw R) : 0 + p = p.trim := by
  apply congrArg trim
  ext <;> simp [add_size, add_coeff, *]

theorem add_zero_trim [LawfulBEq R] (p : CPolynomialRaw R) : p + 0 = p.trim := by
  apply congrArg trim
  ext <;> simp [add_size, add_coeff, *]
...
lemma foldl_preserves_canonical {α : Type*}
    (f : CPolynomialRaw R → α → CPolynomialRaw R)
    (z : CPolynomialRaw R) (as : Array α)
    (hz : z.trim = z)
...
lemma coeff_foldl_add [LawfulBEq R]
    (l : List α)
    (f : α → CPolynomialRaw R)
    (z : CPolynomialRaw R) (k : ℕ) :
    (l.foldl (fun acc x => acc + f x) z).coeff k
      ...
      · simp
      · simp +zetaDelta at *
        convert congr_arg₂ ( · + · ) ( ‹∀ z : CPolynomialRaw R, (
          List.foldl ( fun ( acc : CPolynomialRaw R ) ( x : α ) => acc + f x ) z l )[ k ]?.getD 0
...
-/
lemma sum_zipIdx_eq_sum_range {α : Type*} [AddCommMonoid α] (p : CPolynomialRaw R) (f : R → ℕ → α) :
...
Computes double sum.
-/
theorem double_sum_eq [LawfulBEq R] (p q r : CPolynomialRaw R) (n : ℕ) :
...
-/
lemma sum_range_extend  (p q : CPolynomialRaw R) (k : ℕ) :
...
lemma sum_range_reverse [LawfulBEq R] (p q : CPolynomialRaw R) (k : ℕ) :
...
theorem nsmul_zero [LawfulBEq R] (p : CPolynomialRaw R) : nsmul 0 p = 0 := by
...
theorem nsmulRawSucc (n : ℕ) (p : CPolynomialRaw Q) :
...
theorem nsmul_succ [LawfulBEq R] (n : ℕ) {p : CPolynomialRaw R} :
...
theorem neg_trim [LawfulBEq R] (p : CPolynomialRaw R) : p.trim = p → (-p).trim = -p := by
  apply Trim.non_zero_map
  simp

theorem neg_add_cancel [LawfulBEq R] (p : CPolynomialRaw R) : -p + p = 0 := by
  rw [← zero_canonical]
...
/--
Helper lemma for mul_assoc.
Coefficient after smul.
-/
lemma smul_coeff [LawfulBEq R] (a : R) (p : CPolynomialRaw R) (k : ℕ) :
    (smul a p).coeff k = a * p.coeff k := by
  exact smul_equiv p k a


lemma smul_addRaw_distrib [LawfulBEq R] :
    ∀ (a' : R) (q r : CPolynomialRaw R), smul a' (q.addRaw r)
...
-/
lemma coeff_smul_add_distrib [LawfulBEq R] (a : R) (q r : CPolynomialRaw R) (i : ℕ) :
    (smul a (q + r)).coeff i = (smul a q).coeff i + (smul a r).coeff i := by
      ...
      have h_expand : (smul a (q + r)).coeff i = a * ((q + r).coeff i) ∧
                         ((smul a q).coeff i) + ((smul a r).coeff i) =
                           ...
                           have h_expand : ∀ (p : CPolynomialRaw R), (smul a p).coeff i
                              = a * p.coeff i := by
...
-/
lemma coeff_add_smul [LawfulBEq R]  (a b : R) (q : CPolynomialRaw R) (k : ℕ) :
    (smul (a + b) q).coeff k = (smul a q).coeff k + (smul b q).coeff k := by
      have h_distrib : ∀ (a b : R) (q : CPolynomialRaw R) (k : ℕ),
...
lemma coeff_sum : ∀ (p : CPolynomialRaw R) (k : ℕ),
...
The coefficient of `p * q` at index `k` is the sum of the coefficients of the terms
 `(a_i * q) * X^i`.
-/
lemma coeff_mul [LawfulBEq R] (p q : CPolynomialRaw R) (k : ℕ) :
    (p * q).coeff k = (p.zipIdx.toList.map (fun ⟨a, i⟩ =>
      ...
      · congr
        ...
        have h_mul_def : ∀ (p q : CPolynomialRaw R), p * q
...
-/
lemma coeff_mulPowX [LawfulBEq R] (i : ℕ) (p : CPolynomialRaw R) (k : ℕ) :
...
/--
Helper lemma for mul_assoc.
Coefficient mutliplication by X, similar to coeff_mulPowX.
-/
lemma coeff_mulPowX' [LawfulBEq R] (p : CPolynomialRaw R) (n i : ℕ) :
...
Coefficient of mulPowX.
 -/
lemma mulPowX_coeff' [LawfulBEq R] (p : CPolynomialRaw R) (n k : ℕ) :
...
Distributivity of scalar multiplication over polynomial addition at the coefficient level
-/
lemma coeff_mulPowX_smul_add [LawfulBEq R] (i : ℕ) (a b : R) (q : CPolynomialRaw R) (k : ℕ) :
...
lemma equiv_mul_one [LawfulBEq R] (p : CPolynomialRaw R) : Trim.equiv (p * 1) p := by
  have h_mul_one : ∀ (p : CPolynomialRaw R), (p * 1).coeff = p.coeff := by
    intro p; funext i
    rw [ show p * 1 = p * 1 from rfl ]
    have mul_one_unwrap : ∀ (p : CPolynomialRaw R), (p * 1).coeff = fun k =>
      ...
      · have h_mul_def : ∀ (p : CPolynomialRaw R), p * 1 =
...
theorem mul_is_trimmed [LawfulBEq R] (p q : CPolynomialRaw R) : (p * q).trim = p * q := by
...
-/
lemma mul_eq_foldl (p q : CPolynomialRaw R) :
...
-/
lemma smul_mulPowX_coeff [LawfulBEq R] (a : R) (q : CPolynomialRaw R) (i k : ℕ) :
...
-/
lemma mul_coeff_list [LawfulBEq R] (p q : CPolynomialRaw R) (k : ℕ) :
...
The coefficient of `p * q` at index `k`, as a sum over 0..p.size-1
 -/
lemma mul_coeff_range_size [LawfulBEq R] (p q : CPolynomialRaw R) (k : ℕ) :
...
-/
lemma mul_coeff [LawfulBEq R] (p q : CPolynomialRaw R) (k : ℕ) :
...
-/
lemma mul_assoc_coeff_rhs [LawfulBEq R] (p q r : CPolynomialRaw R) (n : ℕ) :
    (p * (q * r)).coeff n =
      (Finset.range (n + 1)).sum (fun i =>
...
The coefficient of `(p * q) * r` at index `n`.
-/
lemma mul_mul_coeff [LawfulBEq R] (p q r : CPolynomialRaw R) (n : ℕ) :
    ((p * q) * r).coeff n =
      (Finset.range (n + 1)).sum (fun j =>
        (Finset.range (j + 1)).sum (fun i =>
          p.coeff i * q.coeff (j - i) * r.coeff (n - j))) := by
 ...
 -/
lemma mul_assoc_coeff [LawfulBEq R] (p q r : CPolynomialRaw R) (n : ℕ) :
...
-/
lemma mul_assoc_equiv [LawfulBEq R] (p q r : CPolynomialRaw R) :
...
protected theorem mul_zero [LawfulBEq R] (p : CPolynomialRaw R) : p * 0 = 0 := by
...
protected theorem zero_mul [LawfulBEq R] (p : CPolynomialRaw R) : 0 * p = 0 := by
...
theorem mul_one_trim [LawfulBEq R] (p : CPolynomialRaw R) : p * 1 = p.trim := by
...
theorem one_mul_trim [LawfulBEq R] (p : CPolynomialRaw R) : 1 * p = p.trim := by
  have h_mul_def : ∀ (a b : CPolynomialRaw R),
      a.mul b = (a.zipIdx.foldl (fun acc ⟨a', i⟩ => acc.add ((smul a' b).mulPowX i)) (mk #[])) :=
        by exact fun a b => rfl
  have : 1 * p = (mk #[1] : CPolynomialRaw R).mul p := by rfl
  rw [this, h_mul_def]
  show (mk #[1]).zipIdx.foldl (fun acc ⟨a', i⟩ => acc.add ((smul a' p).mulPowX i)) (mk #[])
      = p.trim
  conv_lhs => rw [show (mk #[1] : CPolynomialRaw R).zipIdx = #[(1, 0)] by rfl]
  rw [show Array.foldl (fun acc ⟨a', i⟩ => acc.add ((smul a' p).mulPowX i)) (mk #[]) #[(1, 0)] =
           (mk #[] : CPolynomialRaw R).add ((smul 1 p).mulPowX 0) by rfl]
...
protected theorem left_distrib [LawfulBEq R] (p q r : CPolynomialRaw R) :
    p * (q + r) = p * q + p * r := by
      have h_eq : p * (q + r) = p * q + p * r ↔ p * (q + r) = (p * q + p * r).trim := by
        ...
        have h_add_trim : ∀ (p q : CPolynomialRaw R), (p + q).trim = p.trim + q.trim := by
          intros p q
          have h_add_trim : ∀ (p q : CPolynomialRaw R), (p + q).trim = p.trim + q.trim := by
            intros p q
            have h_add_trim : ∀ (p q : CPolynomialRaw R), (p + q).coeff
...
protected theorem right_distrib [LawfulBEq R] (p q r : CPolynomialRaw R) :
    (p + q) * r = p * r + q * r := by
...
protected theorem mul_assoc [LawfulBEq R] (p q r : CPolynomialRaw R) : p * q * r = p * (q * r) := by
  apply Trim.canonical_ext
  · exact mul_is_trimmed (p * q) r
  · exact mul_is_trimmed p (q * r)
...
lemma mul_coeff_comm [LawfulBEq R] (p q : CPolynomialRaw R) (k : ℕ) :
    (Finset.range (k + 1)).sum (fun i => p.coeff i * q.coeff (k - i)) =
...
lemma mul_comm_coeff [LawfulBEq R] (p q : CPolynomialRaw R) (k : ℕ) :
...
lemma mul_comm_equiv [LawfulBEq R] (p q : CPolynomialRaw R) :
    Trim.equiv (p * q) (q * p) := by
  intro i
  exact mul_comm_coeff p q i

/-- Commutativity of multiplication for CPolynomialRaw over a CommRing. -/
protected theorem mul_comm [LawfulBEq R] (p q : CPolynomialRaw R) : p * q = q * p := by
  apply Trim.canonical_ext
...
instance [LawfulBEq R] : AddCommSemigroup (CPolynomialRaw R) where
