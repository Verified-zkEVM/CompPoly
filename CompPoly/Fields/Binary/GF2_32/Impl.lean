...
/-!
# GF(2^32) Executable Bit-Vector Operations

Executable operations for the `GF(2^32)` candidate polynomial
`X ^ 32 + X ^ 22 + X ^ 2 + X + 1`.
...
certificate supplied by `GF2_32.Primitive`.
-/
...
/-- Executable parameters for the `GF(2^32)` candidate polynomial. -/
def params : BinaryField.Extension.ExecutableParams where
  degree := extensionDegree
  tailExponents := [0, 1, 2, 22]
  tailExponents_bound := by
    intro exponent h
    simp at h
    rcases h with rfl | rfl | rfl | rfl <;> unfold extensionDegree <;> omega

/-- The executable tail list denotes the low-degree part of the GF32 polynomial. -/
...
/-- Executable bit-vector carrier for `GF(2^32)`. -/
@[reducible]
...
def toBitVec : ConcreteField → B32 :=
  BinaryField.Extension.Concrete.toBitVec Concrete.params

def ofBitVec : B32 → ConcreteField :=
...
def ofNat (x : Nat) : ConcreteField :=
  BinaryField.Extension.Concrete.ofNat Concrete.params x
...
/-- Certified executable field structure for `GF(2^32)`. -/
    ...
    { toDivisionRing :=
        { toRing :=
            { add := BinaryField.Extension.Concrete.add Concrete.params
              ...
              nsmul := fun m x =>
                if m % 2 = 0 then BinaryField.Extension.Concrete.zero Concrete.params else x
              zsmul := fun m x =>
                if m % 2 = 0 then BinaryField.Extension.Concrete.zero Concrete.params else x
              npow := fun m x => BinaryField.Extension.Concrete.pow Concrete.params x m
              ...
              npow_succ := by
                intro m x
                change BinaryField.Extension.Concrete.pow Concrete.params x (m + 1) =
                  BinaryField.Extension.Concrete.mul Concrete.params
                    (BinaryField.Extension.Concrete.pow Concrete.params x m) x
                exact BinaryField.Extension.Concrete.pow_succ_of_spec Concrete.params
                  (data := definingPolynomialData) hP hP_degree hDegree_pos m x }
...
/-- The raw executable carrier has `2^32` bit-patterns. -/
theorem card :
...
/-- Ring homomorphism from executable GF32 elements to the quotient specification. -/
...
/-- Algebra homomorphism from executable GF32 elements to the quotient specification. -/
...
/-- Algebra equivalence between executable GF32 elements and the quotient specification. -/
