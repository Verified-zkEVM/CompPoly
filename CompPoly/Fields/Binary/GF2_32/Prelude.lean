...
Irreducibility and primitive-order certificates are added in later `GF2_32`
...
/-- The tail has natural degree `22`. -/
lemma definingTail_natDegree :
    definingTail.natDegree = 22 := by
  unfold definingTail
  compute_degree!
...
/-- Candidate defining polynomial for `GF(2^32)`. -/
...
/-- The candidate defining polynomial has natural degree `32`. -/
...
/-- The candidate defining polynomial has degree `32`. -/
...
lemma definingPolynomial_eq_P_val :
  ...
  rw [BinaryField.Extension.toPoly_one_shiftLeft 32 (by omega)]
  rw [BinaryField.Extension.toPoly_one_shiftLeft 22 (by omega)]
  rw [BinaryField.Extension.toPoly_one_shiftLeft 2 (by omega)]
  rw [BinaryField.Extension.toPoly_one_shiftLeft 1 (by omega)]
  rw [(show (1 : BitVec 64) = 1 <<< 0 by simp)]
...
/-- Smooth subgroup refinement schedule for `2^32 - 1`. -/
def smoothRootSchedule : Array Nat :=
  #[3, 5, 17, 257, 65537]

/-- The GF32 smooth schedule refines the multiplicative group down to singleton cosets. -/
