...
/-!
# GF(2^32) Primitive-Order Scaffold
...
/-- Distinct prime factors of `2^32 - 1`. -/
def primitivePrimeFactors : List Nat :=
  [3, 5, 17, 257, 65537]

/-- Factorization of the multiplicative-group order `2^32 - 1`. -/
lemma multiplicativeGroupOrder_factorization :
    multiplicativeGroupOrder = 3 * 5 * 17 * 257 * 65537 := by
...
/-- The GF2_32 candidate polynomial passes Rabin's irreducibility test. -/
theorem definingPolynomial_irreducible : Irreducible definingPolynomial :=
  BinaryField.Extension.irreducible_of_rabin_32_passed_over_GF2
    definingPolynomial
    (by simpa [extensionDegree] using definingPolynomial_natDegree)
    X_pow_2_pow_32_add_X_dvd
...
/-- Local kernel-safe primality certificate for the largest GF32 group-order factor. -/
