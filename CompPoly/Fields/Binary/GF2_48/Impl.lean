...
/-!
# GF(2^48) Executable Bit-Vector Operations

Executable operations for the `GF(2^48)` candidate polynomial
`X^48 + X^9 + X^7 + X^4 + 1`.
...
certificate supplied by `GF2_48.Primitive`.
-/
...
/-- Executable bit-vector carrier for `GF(2^48)`. -/
@[reducible]
...
def toBitVec : ConcreteField → B48 :=
  BinaryField.Extension.Concrete.toBitVec Concrete.params

def ofBitVec : B48 → ConcreteField :=
...
/-- Certified executable field structure for `GF(2^48)`. -/
...
/-- The raw executable carrier has `2^48` bit-patterns. -/
theorem card :
...
/-- Ring homomorphism from executable GF48 elements to the quotient specification. -/
...
/-- Algebra homomorphism from executable GF48 elements to the quotient specification. -/
...
/-- Algebra equivalence between executable GF48 elements and the quotient specification. -/
