...
Irreducibility and primitive-order certificates are added in later `GF2_64`
...
/-- The tail has natural degree `4`. -/
lemma definingTail_natDegree :
    definingTail.natDegree = 4 := by
  unfold definingTail
  compute_degree!
...
/-- Candidate defining polynomial for `GF(2^64)`. -/
...
/-- The candidate defining polynomial has natural degree `64`. -/
...
/-- The candidate defining polynomial has degree `64`. -/
...
lemma definingPolynomial_eq_P_val :
  ...
  rw [BinaryField.Extension.toPoly_one_shiftLeft 64 (by omega)]
  ...
  rw [(show (1 : BitVec 128) = 1 <<< 0 by simp)]
...
/-- Smooth subgroup refinement schedule for `2^64 - 1`. -/
def smoothRootSchedule : Array Nat :=
  #[3, 5, 17, 257, 641, 65537, 6700417]

/-- The GF64 smooth schedule refines the multiplicative group down to singleton cosets. -/
