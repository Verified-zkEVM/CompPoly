...
/-!
# GF(2^64) Executable Bit-Vector Operations

Executable operations for the `GF(2^64)` candidate polynomial
`X ^ 64 + X ^ 4 + X ^ 3 + X + 1`.
...
certificate supplied by `GF2_64.Primitive`.
-/
...
/-- Executable bit-vector carrier for `GF(2^64)`. -/
@[reducible]
...
def toBitVec : ConcreteField → B64 :=
  BinaryField.Extension.Concrete.toBitVec Concrete.params

def ofBitVec : B64 → ConcreteField :=
...
/-- Certified executable field structure for `GF(2^64)`. -/
...
/-- The raw executable carrier has `2^64` bit-patterns. -/
theorem card :
...
/-- Ring homomorphism from executable GF64 elements to the quotient specification. -/
...
/-- Algebra homomorphism from executable GF64 elements to the quotient specification. -/
...
/-- Algebra equivalence between executable GF64 elements and the quotient specification. -/
