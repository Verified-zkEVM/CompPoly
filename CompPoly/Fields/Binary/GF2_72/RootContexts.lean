...
/-!
# GF(2^72) Root-Search Contexts

Certified finite-field context for the executable `GF(2^72)` carrier.
-/
...
/-- Finite-field context for the executable `GF(2^72)` carrier. -/
...
/-- Algebraic trace value embedded back into executable `GF(2^72)`. -/
...
/-- The executable GF72 carrier has dimension `72` over `GF(2)`. -/
...
/-- The GF72 trace lands in the embedded `GF(2)` constants. -/
...
/-- The transported quotient power basis for executable `GF(2^72)`. -/
...
theorem polynomialBasisBasis_apply (i : Fin extensionDegree) :
  ...
  rw [map_pow]
  change toSpecAlgEquiv.symm GF2_72.root ^
      ((finCongr polynomialPowerBasis_dim_eq).symm i).val = root ^ i.val
  have hrootSymm : toSpecAlgEquiv.symm GF2_72.root = root := by
...
/-- The executable polynomial basis separates distinct elements by GF72 trace coordinates. -/
...
/-- Shoup trace splitter context for `GF(2^72)`. -/
...
/-- Smooth cyclic splitter context for `GF(2^72)` using Horner leaf evaluation. -/
def smoothHornerRootContext :
    CPolynomial.Roots.FiniteField.SmoothCyclicRootContext ConcreteField :=
  smoothCyclicRootContextWith (CPolynomial.BatchEvalContext.horner ConcreteField)

/-- Smooth cyclic splitter context for `GF(2^72)` using subproduct-tree leaf evaluation. -/
