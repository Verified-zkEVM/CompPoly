...
monic-modulus side of the extension framework (`CompPoly.Extension.GeneralParams`).
...
* `KoalaBear.ext5Params`: the `GeneralParams` for `X^5 + X^2 - 1`.
...
def ext5Params : GeneralParams Field where
  d := 5
  two_le := by norm_num
  lower := #v[-1, 0, 1, 0, 0]
  q := fieldSize
...
theorem ext5Params_poly : ext5Params.poly = quinticPoly := by
  ...
  rw [GeneralParams.poly, quinticPoly]
